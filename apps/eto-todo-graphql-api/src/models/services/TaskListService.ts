...
export class TaskListService {
  ...
  private doesTaskBelongToPeriod(
    task: Task,
    date: Date,
    dateType: DateType
  ): boolean {
    if (task.isRecurrent) {
      ...
      return (
        ...
        (task.specificDateType === DateType.Date ||
          (task.specificDateType === DateType.Week &&
            task.recurrentDateValue === date.getDay() + 1) ||
          (task.specificDateType === DateType.Month &&
            task.recurrentDateValue === date.getDate()))
      );
    } else {
