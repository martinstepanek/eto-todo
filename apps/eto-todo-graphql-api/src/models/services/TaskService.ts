...
export class TaskService {
  ...
  public async getMarkedTasksByListType(
    taskListType: TaskListType,
    forUser: User
  ): Promise<Task[]> {
    return this.taskEntryService.markFor(
      await this.getTasksByListType(taskListType, forUser),
      taskListType
    );
  }

  private async getTasksByListType(
    taskListType: TaskListType,
    forUser: User
  ): Promise<Task[]> {
    switch (taskListType) {
      case TaskListType.Today:
        return this.getTasksForDay(
          forUser,
          DateHelper.getDateForListType(taskListType)
        );
  ...
  private async getTasksForDay(forUser: User, dayDate: Date): Promise<Task[]> {
    ...
    tasks = [
      ...tasks,
      ...recurrentTasksDaily,
      ...recurrentTasksInWeek,
      ...recurrentTasksInMonth,
    ];
