...
export class TaskResolver {
  public constructor(
    ...
    @InjectRepository()
    private readonly taskEntryRepository: TaskEntryRepository,
    @Inject('TaskService') private readonly taskService: TaskService,
    @Inject('TaskEntryService')
    private readonly taskEntryService: TaskEntryService,
  ...
  public async createTask(
    ...
    if (!taskInput.isRecurrent && taskInput.specificDateValue == null) {
      throw new Error(
        'When task is not recurrent, specificDateValue is required'
      );
    }
  ...
  public async tasks(
    ...
    return this.taskService.getMarkedTasksByListType(
      listType,
      ctx.userIdentity.user
    );
  }

  @Authorized()
  @Mutation(() => TaskOperation, {
    nullable: true,
    description: 'Mark task as done',
  })
  public async markTaskAsDone(
    @Arg('taskEntry') taskEntryInput: TaskEntryInput
  ): Promise<TaskOperation> {
    const task = await this.taskRepository.findOne({
      where: {
    ...
    const entry = await this.taskEntryService.findEntry(
      task,
      taskEntryInput.when
    );
  ...
  @Mutation(() => TaskOperation, {
    nullable: true,
    description: 'Mark task as not done',
  })
  public async markTaskAsNotDone(
    @Arg('taskEntry') taskEntryInput: TaskEntryInput
  ): Promise<TaskOperation> {
    const task = await this.taskRepository.findOne({
      where: {
    ...
    const start = DateHelper.getStartOfPeriod(
      taskEntryInput.when,
      task.specificDateType
    );
    const end = DateHelper.getEndOfPeriod(
      taskEntryInput.when,
      task.specificDateType
    );
